...
class bwDB:
    ...
    def sql_query(self, sql, params = ()):
        '''
            ...
            generator method for containing SQL
                sql is string containing SQL
                params is list containing parameters
    ...
    def insert(self, record):
        '''
            ...
            omit id clumn to let SQLite generate it
        '''
        klist = sorted(record.keys())
        values = [ record[v] for v in klist ] # a list of values ordered by key
    ...
    def update(self, ID, record):
        '''
            ...
            update a row in the table
                id is the value of the id colum for the row to be updated
                record is a dict with key/value pairs corresponding to table schema
        '''
        klist = sorted(record.keys())
        values = [ record[v] for v in klist ] # a list of values ordered by key
        ...
        q = 'UPDATE {} SET {} WHERE id = ?'.format(
            self.table,
            ', '.join(map(lambda string: '{} = ?'.format(string), klist))     
        )
...
def test():
    ...
    for r in records: db.insert(r)
    print('Done.')
                
    print('Read from table')
    for r in db.getrecords(): print(dict(r))
